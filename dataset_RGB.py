...
class DataLoaderTrain(Dataset):
    def __init__(self, img_dir):
        ...
        inp_ir_files = sorted(os.listdir(os.path.join(img_dir, 'ir')))
        inp_rgb_files = sorted(os.listdir(os.path.join(img_dir, 'rgb')))
        self.inp_ir_filenames = [os.path.join(img_dir, 'ir', x)  for x in inp_ir_files if is_image_file(x)]
        self.inp_rgb_filenames = [os.path.join(img_dir, 'rgb', x) for x in inp_rgb_files if is_image_file(x)]

        self.sizex       = len(self.inp_ir_filenames)  # get the size of target
    ...
    def __getitem__(self, index):
        ...
        filename = os.path.splitext(os.path.split(inp_ir_path)[-1])[0]

        return inp_ir_img, inp_rgb_img, filename

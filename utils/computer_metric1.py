...
def metric(ir_dir, vi_dir, fus_dir, save_dir, epoch):
    with_mean = True
    method = 'MMFusion'
    ...
    SSIM_list = []
    filename_list = ['']
    fus_method_dir = os.path.join(fus_dir)

    metric_save_name = os.path.join(save_dir, 'metric_{}_{}_{}.xlsx'.format(dataset_name, method, epoch))
    ...
    for _, item in enumerate(eval_bar):
        ...
        # print(ir_name)
        EN, SD, SF, AG = evaluation_one(ir_name, vi_name, f_name)
        EN_list.append(EN)
        SD_list.append(SD)
        SF_list.append(SF)
        AG_list.append(AG)
    ...
    if with_mean:
        ...
        print(
            "Avg metric: \n  Avg_EN:%.4f, Avg_SD:%.4f, Avg_SF:%.4f, Avg_AG:%.4f"
            % (Avg_EN, Avg_SD, Avg_SF, Avg_AG))
...
#     with_mean = True
#     dataset_name = 'mfnet'
#     ir_dir = os.path.join('../dataset/test/mfnet/ir')
#     vi_dir = os.path.join('../dataset/test/mfnet/rgb')
#     fus_dir = os.path.join('/mnt/disk/Compared_methods/mfnet/test_results')
...
#         # metric_save_name = os.path.join(save_dir, 'Text_IF_rgb.xlsx')
#         filelist = natsorted(os.listdir(fus_method_dir))
#         eval_bar = tqdm(filelist)
#         for _, item in enumerate(eval_bar):
#             ir_name = os.path.join(ir_dir, item)
